...
/**
 * Region interaction tests for WorldMapViewModel.
 * Tests region tap, selection, and exploration functionality.
 */
@OptIn(ExperimentalCoroutinesApi::class)
class WorldMapViewModelRegionTest {
    ...
    @Test
    fun `onRegionTap calls exploreRegionUseCase with correct regionId`() =
        runTest {
            // Given
            setupReadyState()
            val exploreResult =
            ...
            // Then - verify the regionId mapping from islandId to regionId
            coVerify { exploreRegionUseCase(testUserId, "make_atoll") }
        }

    @Test
    fun `onRegionTap updates exploration state on new discovery`() =
        runTest {
            // Given
            setupReadyState()
            val exploreResult =
            ...
            // Then
            val state = viewModel.uiState.value as WorldMapUiState.Ready
            assertEquals(2, state.explorationState?.totalDiscoveries)
            assertTrue(state.explorationState?.hasExplored("make_atoll") == true)
        }
    ...
    @Test
    fun `getExplorationPercentage returns 0 when no regions explored`() =
        runTest {
            // Given - no explored regions
            val emptyExplorationState =
                testExplorationState.copy(
                    exploredRegions = emptySet(),
                    totalDiscoveries = 0,
                )
            every { getWorldMapStateUseCase.observe(testUserId) } returns flowOf(testWorldMapState)
            coEvery { exploreRegionUseCase.getExplorationState(testUserId) } returns emptyExplorationState
