...
/**
 * Unit tests for UpdateGameStateUseCase - Game Flow Actions
 * Tests game state transitions for StartGame, PauseGame, ResumeGame, ExitGame, ResetGame, RestartGame
 */
class UpdateGameStateUseCaseGameFlowTest {
    ...
    fun `handlePauseGame returns Paused state when current state is Playing`() {
        ...
        // Verify the previous state has the expected properties (not same reference due to copy())
    ...
    fun `handlePauseGame updates elapsed time correctly`() {
        ...
        // Then
        val pausedState = result as MatchGameState.Paused
        // Elapsed time should be previous elapsed + time since start
        assertTrue(pausedState.previousState.elapsedTime >= 500L)
    }
